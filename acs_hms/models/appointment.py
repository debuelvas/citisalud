...
class Appointment(models.Model):
    ...
    def get_procudures_to_invoice(self):
        """
        Optimizado: Busca procedimientos relacionados sin factura.
        Sin @api.depends porque no hay campo directo en appointment.
        """
        for rec in self:
            # Buscar procedimientos relacionados con esta cita que no tengan factura
            procedures = self.env['acs.patient.procedure'].search([
                ('appointment_ids', 'in', rec.id),
                ('invoice_id', '=', False)
            ])
            rec.procedure_to_invoice_ids = procedures

    @api.depends('department_id')
    def acs_get_department(self):
        """
        Optimizado: Agregado @api.depends, eliminada búsqueda innecesaria
        """
        for rec in self:
            # OPTIMIZACIÓN: No es necesario hacer search si ya tenemos el registro
            rec.acs_department_id = rec.department_id.id if rec.department_id else False
    ...
    @api.depends('date', 'date_to', 'manual_planned_duration')
    def _get_planned_duration(self):
        """
        Optimizado: Agregado manual_planned_duration en depends
        CORREGIDO: Evita recursión condicional innecesaria
        """
        for rec in self:
            if rec.date and rec.date_to:
                diff = rec.date_to - rec.date
                rec.planned_duration = (diff.days * 24) + (diff.seconds/3600)
            else:
                rec.planned_duration = rec.manual_planned_duration or 0.0
