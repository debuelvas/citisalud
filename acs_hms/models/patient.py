...
class ACSPatient(models.Model):
    ...
    @api.depends('treatment_ids', 'appointment_ids', 'evaluation_ids', 'patient_procedure_ids')
    def _rec_count(self):
        """
        Optimizado: Agregado @api.depends
        NOTA: prescription_count usa search_count porque no existe relación prescription_ids en hms.patient
        """
        rec = super(ACSPatient, self)._rec_count()
        for rec in self:
            # prescription_count: Sin relación One2many, usar search_count
            rec.prescription_count = self.env['prescription.order'].search_count([('patient_id', '=', rec.id)])

            # Campos con relaciones One2many existentes
    ...
    @api.depends('appointment_ids', 'appointment_ids.state', 'appointment_ids.date')
    def acs_check_cancellation_flag(self):
        """
        Optimizado: Eliminada búsqueda en BD, usa relación appointment_ids
        """
        ...
        for rec in self:
            # OPTIMIZACIÓN: Filtrar desde appointment_ids en lugar de search_count
            cancelled_appointments = rec.appointment_ids.filtered(
                lambda a: a.date >= date_start and
                         a.date <= date_end and
                         a.state == 'cancel'
            )
            cancelled_count = len(cancelled_appointments)

            rec.show_cancellation_warning_flag = cancelled_count >= acs_flag_count_limit
            rec.acs_flag_days = acs_flag_days
            rec.acs_cancelled_appointments = cancelled_count
