...
class DianApplicationResponse(models.Model):
    ...
    recepcion_id = fields.Many2one('recepcion.factura.dian')
    ...
    def _get_common_template_vals(self):
        ...
        return {
                ...
                "SenderPartyName": self._replace_character_especial(self.company_id.partner_id.name),
                "SenderSchemeID": self.company_id.partner_id.dv,
                "SenderSchemeName": self.return_number_document_type(self.company_id.partner_id.l10n_co_document_code),
                ...
                "PersonSchemeName": self.return_number_document_type(self.user_id.partner_id.l10n_co_document_code),
                "PersonID": self.user_id.partner_id.vat_co,
                "PersonFirstName": self._replace_character_especial(self.user_id.partner_id.firs_name or self.user_id.partner_id.name),
                "PersonFamilyName": self._replace_character_especial(self.user_id.partner_id.first_lastname or self.user_id.partner_id.name),
                "PersonJobTitle": self._replace_character_especial(self.user_id.partner_id.function) or 'Auxiliar',
    ...
    @api.model
    def generate_from_attached_document(self, attached_document_xml, response_type,ideve):
        """
        :param: attached_document_xml: string
        :param: response_type: string enum: see the field response_code of this model
        """
        data = self.env['recepcion.factura.dian'].search([('id', '=', ideve)])
        ...
        if response_type == '034':
            partner_tribute_name = data_ad['AttachedDocument']['cac:SenderParty']['cac:PartyTaxScheme']['cbc:CompanyID']['@schemeName']
        ...
        if response_type == '031':
            reclamacion = data.supplier_claim_concept
            reclamacion_text = self.calculate_text_supplier_claim_concept(reclamacion)
            template_vals.update({
                'reclamacion': 'name="{}" listID="{}"'.format(reclamacion_text, reclamacion)
                })
        template_vals.update({
    ...
    def generate_from_electronic_invoice(self, id_e, response_type):
        ...
        :param: response_type: string enum: see the field response_code of this model
        """
        data = self.env['recepcion.factura.dian'].search([('id', '=', id_e)])
        
        document_referenced = data.n_invoice 
        doc_adq = data.supplier_id.vat
        vat = ''
        dv = ''
        try:
            doc_adq = data.supplier_id.vat
            ...
            vat = ''
            dv = ''
            _logger.exception("Ocurrió un error al procesar el campo 'vat': %s", str(e)) 
        number = ""
        ...
        if data.supplier_id.type_regime_id.code == '48':
            CustomerTaxSchemeID = '01'
            CustomerTaxSchemeName = 'IVA'
        elif data.supplier_id.type_regime_id.code == '49':
            CustomerTaxSchemeID = 'ZZ'
            CustomerTaxSchemeName = 'No Aplica'
        else:
            raise UserError('el Tercero no tiene Un regimen configurado')
        template_vals = rec._get_common_template_vals()
        if response_type == '034':
            partner_tribute_name = CustomerTaxSchemeName
            template_vals.update({
                'notes': 'Manifiesto bajo la gravedad de juramento que transcurridos 3 días hábiles siguientes a la fecha de recepción de la mercancía o del servicio en la referida factura de este evento, el adquirente {} identificado con NIT {} no manifestó expresamente la aceptación o rechazo de la referida factura, ni reclamó en contra de su contenido.'.format(partner_tribute_name, doc_adq)
            })
        if response_type == '031':
        ...
        template_vals.update({
                "CustomerPartyName": self.remove_accents(data.supplier_id.name), 
                "CustomerschemeID":  dv, #data.partner_id.dv, 
                "CustomerID": self.return_number_document_type(data.supplier_id.l10n_co_document_code) or "31",
                "CustomercompanyIDtext": vat,
                "InvoiceID": document_referenced,
                "CustomerTaxSchemeID": CustomerTaxSchemeID, #data.partner_id.tribute_id.code or "01", 
                "CustomerTaxSchemeName": CustomerTaxSchemeName, #data.partner_id.tribute_id.name or  "IVA",
                "UUIDinvoice": data.cufe, 
            })
...
<ApplicationResponse xmlns="urn:oasis:names:specification:ubl:schema:xsd:ApplicationResponse-2"
    ...
    <cac:DocumentResponse>
        <cac:Response>
            <cbc:ResponseCode %(reclamacion)s>%(ResponseCode)s</cbc:ResponseCode>
...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:wcf="http://wcf.dian.colombia">
    ...
    def remove_accents(self,chain):
        regex_pattern = r'[^a-zA-Z0-9\s]'  # Esto filtra todos los caracteres que no sean letras, números o espacios
        cleaned_chain = re.sub(regex_pattern, '', chain)
        cleaned_chain = ''.join((c for c in unicodedata.normalize('NFD', cleaned_chain) if unicodedata.category(c) != 'Mn'))
